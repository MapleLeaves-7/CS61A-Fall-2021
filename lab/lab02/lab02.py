...
def composite_identity(f, g):
    ...
    False
    """
    # Don't even need these two lines
    def compose_fg(x): return f(g(x))
    def compose_gf(x): return g(f(x))
    return lambda x: compose_fg(x) == compose_gf(x)
    # Can just return lambda x: f(g(x)) == g(f(x))
