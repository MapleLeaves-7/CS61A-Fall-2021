...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            dice_roll0 = take_turn(
                strategy0(score0, score1), score1, dice, goal)
            score0 += dice_roll0
            score0 += hog_pile(score0, score1)
        else:
            dice_roll1 = take_turn(
                strategy1(score1, score0), score0, dice, goal)
            score1 += dice_roll1
    ...
    def commentary(score0, score1):
        if who == 0:
            ...
            if gain > running_high:
                print(f"{gain} point(s)! That's a record gain for Player 0!")
                return announce_highest(0, score0, gain)
            return announce_highest(0, score0, running_high)
        else:
            gain = score1 - last_score
            if gain > running_high:
                print(f"{gain} point(s)! That's a record gain for Player 1!")
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    i = 1
    while i <= 10:
        current_average = averaged_dice(i, dice)
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    do_picky_piggy = picky_piggy_strategy(
        score, opponent_score, cutoff, num_rolls)
    if do_picky_piggy == 0:
        return 0
    new_score = score + picky_piggy(opponent_score)
    # This mean Hog Pile is triggered
